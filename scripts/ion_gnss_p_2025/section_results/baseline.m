...
for row = 1:3
    for col = 1:2
        ...
        for ia = 1:numel(title_names)
            ...
            init_est = data.(mdl).(sev).init_csm;
        ...
        if row == 1
            plot(ax, time_zoom, zeros(size(idx_full)),'LineWidth',2.5,'Color','w','DisplayName','Target Error');
        end
        if strcmp(mdl, 'cpssm_w_refr')
            ...
            if row == 3
                h_diff = plot(ax, time_zoom, diffractive_phase, '-.','LineWidth',1.5,'Color','w','DisplayName','Diffractive Phase $\phi_{\mathrm{D}, 1} [k]$');
