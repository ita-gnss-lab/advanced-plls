    ...
    for severity = severities
        for seed = 1:mc_seeds
            for sigma2_W_3_idx = 1:sigma2_W_3_sweep_amount
                ...
                % non-geometry-aided KF-AR
                data.(refractivity).(severity).kf_ar_no_geo.rmse.delta_phi_T(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_no_geo_valid_hat_phi_T - valid_total_phase));
                data.(refractivity).(severity).kf_ar_no_geo.rmse.delta_phi_LOS(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_no_geo_valid_hat_phi_W - valid_los_phase));
                data.(refractivity).(severity).kf_ar_no_geo.rmse.delta_phi_s(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_no_geo_valid_hat_phi_AR - valid_cpssm_total_phase));
                % geometry-aided KF-AR
                data.(refractivity).(severity).kf_ar_geo.rmse.delta_phi_T(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_geo_valid_hat_phi_T - valid_total_phase));
                data.(refractivity).(severity).kf_ar_geo.rmse.delta_phi_LOS(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_geo_valid_hat_phi_W - valid_los_phase));
                data.(refractivity).(severity).kf_ar_geo.rmse.delta_phi_s(sigma2_W_3_idx, seed) = rms(wrapToPi(kf_ar_geo_valid_hat_phi_AR - valid_cpssm_total_phase));

                %%%% Saving signals (if the seed is the one defined in `seed_to_save_time_series`)
                if seed == seed_to_save_time_series
                    %%% Save the estimates
                    % non-geometry-aided KF-AR
                    data.(refractivity).(severity).kf_ar_no_geo.sig.phi_T(:, end+1) = kf_ar_no_geo_valid_hat_phi_T;
                    data.(refractivity).(severity).kf_ar_no_geo.sig.phi_W(:, end+1) = kf_ar_no_geo_valid_hat_phi_W;
                    data.(refractivity).(severity).kf_ar_no_geo.sig.phi_AR(:, end+1) = kf_ar_no_geo_valid_hat_phi_AR;
                    % geometry-aided KF-AR
                    data.(refractivity).(severity).kf_ar_geo.sig.phi_T(:, end+1) = kf_ar_geo_valid_hat_phi_T;
                    data.(refractivity).(severity).kf_ar_geo.sig.phi_W(:, end+1) = kf_ar_geo_valid_hat_phi_W;
                    data.(refractivity).(severity).kf_ar_geo.sig.phi_AR(:, end+1) = kf_ar_geo_valid_hat_phi_AR;

                    %%% Save the true phases
                    data.(refractivity).(severity).ground_truth.phi_los_true = valid_los_phase;
                    if is_remove_refractive_effects
                        data.(refractivity).(severity).ground_truth.phi_cpssm_total = valid_cpssm_total_phase;
                    else
                        data.(refractivity).(severity).ground_truth.phi_cpssm_refractive = valid_cpssm_refractive_phase;
                        data.(refractivity).(severity).ground_truth.phi_cpssm_diffractive = valid_cpssm_diffractive_phase;
                    end
                end
